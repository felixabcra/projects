...
contract('Gaming', async (accounts) => {
  ...
  before(async () => {
    ...
    const fundGame = await gaming.fundGame({from: owner, value: web3.utils.toWei('0.01', 'ether')})
  })
  ...
  it('Should record player losses', async() => {
    ...
    assert.isAtLeast(initialBalanceInEther, postBalanceInEther + 0.01, 'Player account should have decreased by the amount of the wager')
  })
